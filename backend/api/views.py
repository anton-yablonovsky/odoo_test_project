...
def delete_building(request, building_id):
    ...
    building.delete()
    return Response(
        {"message": "Building deleted successfully"}, status=status.HTTP_204_NO_CONTENT
    )
...
def delete_entrance(request, entrance_id):
    ...
    try:
        ...
        return Response(
            {"error": "Entrance not found"}, status=status.HTTP_404_NOT_FOUND
        )

    entrance.delete()
    return Response(
        {"message": "Entrance deleted successfully"}, status=status.HTTP_204_NO_CONTENT
    )
...
def delete_apartment(request, apartment_id):
    ...
    apartment.delete()
    return Response(
        {"message": "Apartment deleted successfully"}, status=status.HTTP_204_NO_CONTENT
    )
...
def get_manager_buildings(request):
    ...
    if request.user.role == "manager":  # Ensure user is a manager
        ...
        buildings = (
            request.user.buildings.all()
        )  # Access buildings via the relationship
        print(f"{buildings=}")
        serializer = BuildingSerializer(buildings, many=True)
